...
（1）使用评估环境对风险进行评估
（2）在无法通过的时候使用评估模型来评估
（3）根据attention的值选取需要剪枝的点，减去不影响其他航路的枝叶，并在该点进行智能采样探索，来加速RRTstar的性能
（4）智能采样结果是由神经网络训练的，由于在同一个点智能采样的结果是相同，因此限制智能采样的次数（只进行一次）
（5）采样模型的输入为当前轨迹和场上所有威胁实体的属性
'''
#!/usr/bin/env python3
# coding:utf-8
import random
...
'''
功能：提供RRTstar的规划计算过程，依据地图和评估结果完成基于RRT*的路径规划。
其他功能：支持规划过程展示，和必要信息存储
特殊功能：在环境评估的基础上，观察仿真环境的运行结果
'''
class RRT:
	def __init__(self, mapEva,method,maxIteration,minStep,maxStep,goalProb,nearDis,sceneType,methodofEvaluation,showAnimation,observation,storeData,dimension,stopIteration=None):
		...
		# 如果使用风险来评估轨迹，则生成评估模型对象
		if self.sceneType[0] == "Risk":
			if self.methodofEvaluation == "EvaluationModel":
				self.EvaModel = EvaRoute2.Model(self.mapEva)
			elif self.methodofEvaluation == "Environment":
				self.EvaModel = EnvRoute.Model(self.mapEva)
	...
	def Search(self,pathofPicture = None):
		...
		if ret:
			...
			if self.sceneType[0] == "Obstacle" :
				result = "path cost(distance): "+str(self.pathCost)+ " steps: "+str(len(self.path))+ " time_use: "+str(end_time - start_time)
				...
				if self.stopIteration == None:
					if (Node.distancenn(nnew, self.goalNode) < self.maxStep):
						if self.sceneType[0] == "Risk":
							...
							# print("up:CurrentPath",CurrentPath)
					...
					if dis < self.maxStep:
						if nnew.cost + dis < self.goalNode.cost:
							if self.sceneType[0] == "Risk":
								# print("down:CurrentPath", CurrentPath)
								CurrentPath = self.ObtainCurrentPath(nnew, self.goalNode.x)
								risk, attentionS, sttentionT = self.EvaModel.evalution(CurrentPath)
								self.goalNode.risk, self.goalNode.attentionS, self.goalNode.sttentionT = risk, attentionS, sttentionT
	...
	def Extend(self, nnearest, xrand):
		if self.sceneType[0] == "Obstacle" or self.sceneType[0] == "Risk":
			step = self.maxStep
			...
			if self.sceneType[0] == "Obstacle":
				...
				if self._CollisionLine(xnew, nnearest.x):
					...
					nnew = Node(xnew, parent=nnearest, lcost=dis, children=[])
					return nnew
			elif self.sceneType[0] == "Risk":
				CurrentPath = self.ObtainCurrentPath(nnearest,xnew)
				#print(xnew)
				#plt.plot(xnew[0],xnew[1], "og")
	...
	def ClipPath(self,OriginalPath): #将长轨迹切割
		OP = OriginalPath
		path = [OP[0]] * self.length * self.intervalPoint
		if len(OP) >= self.length * self.intervalPoint:
	...
	def proInformation(self,spacialAtt,temporalAtt,path,enemyEntities):
		...
		#threatPositions = self.getThreatPositions(enemyEntities,oneEneAtt)
		# print(enemyEntities)
		# print(oneEneAtt)
		# print(threatPositions)
	...
	def trimforBranchChildren(self,tree,position,path):
		...
		# 找到选定点的在路径上的孩子节点，并减去所有从该孩子节点延伸的所有节点
		#childrenList = self.fundAllBranchChildren(nnearest,tree,path,nnearest)
		...
		if nnearest == None:
			...
			#print("childrenFinal:", childrenFinal)
			if childrenList:
				for i in childrenList:
					# if self.showAnimation:
					# 	plt.plot(i.x[0],i.x[1], "ok")
					# 	plt.pause(0.0001)
					tree.nodes.remove(i)
					tree.deleteChild(i)
			if self.showAnimation:
	...
	def makeDecision(self,position, pathforSample):
		...
		if self.showAnimation:
			# plt.plot(childrenFinal.x[0], childrenFinal.x[1], "oy")
			# plt.plot(position[0], position[1], "oy")
			plt.plot(xnew[0], xnew[1], "ok", label = "xnew")
			# print(oneEneAtt)
			# print(childrenFinal.x)
			# print(position)
			# print(xnew)
			plt.legend()
	...
	def reParent(self, node, tree):
		...
		if self.sceneType[0] == "Obstacle" :
			...
			for n in nears:
				# print("比较近的单点：", n.x)
				# if not [True for q in Path if all(q == np.array(n.x))]:
				CurrentPath = self.ObtainCurrentPath(n, node.x)
				# print("reparent:CurrentPath:",CurrentPath)
		...
		if self.sceneType[0] == "Obstacle":
			...
			for n in nears:
				...
				if not all(n.x == np.array([0,0])):
					...
					CurrentPath[0] = a
					CurrentPath[1] = b
					#print("reWire:CurrentPath:", CurrentPath)
					newl = Node.distancenn(n, node)
					if node.cost + newl < n.cost and node != n:
						tag, risk, spacialAtt, temporalAtt = self.CollisionRisk(CurrentPath, self.sceneType[1])
						if tag == True:
